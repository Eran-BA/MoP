...
from .gpt_mop import create_gpt_baseline, create_gpt_quartet, create_gpt_mop
...
class GPTComparisonFramework:
    ...
    def build_models(self, vocab_size: int) -> Dict[str, nn.Module]:
        ...
        mop = create_gpt_mop(vocab_size, base_config, 
    ...
    def test_forward_pass(self, batch_size: int = 2, seq_len: int = 64, 
        ...
        for name, details in info.items():
            ...
            if config:
                ...
                if config['use_quartet']:
                    print(f"{'':>10}  Quartet: ✅")
                if details['has_mop']:
                    print(f"{'':>10}  MoP: ✅ (Views: {self.config.n_views}, Kernels: {self.config.n_kernels})")
            else:
