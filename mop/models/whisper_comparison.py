...
from .whisper_mop import (WhisperConfig, create_whisper_baseline,
                          create_whisper_mop)
...
class WhisperComparisonFramework:
    ...
    def build_models(self) -> Dict[str, nn.Module]:
        ...
        # Store models
        self.models = {"baseline": baseline, "mop": mop}
    ...
    def print_comparison_summary(self):
        ...
        for name, details in info.items():
            ...
            if config:
                ...
                print(
                    f"{'':>10}  Audio: {config['n_mels']} mels, {config['n_audio_ctx']} ctx, "
                    f"Vocab: {config['vocab_size']}"
                )
                if details["has_mop"]:
                    print(
                        f"{'':>10}  MoP: ✅ (Views: {self.config.n_views}, "
...
def create_whisper_comparison_framework(
...
if __name__ == "__main__":
    ...
    print("-" * 40)
    audio_results = framework.test_audio_processing(
        batch_size=2, time_steps=64, freq_bins=80
    )
