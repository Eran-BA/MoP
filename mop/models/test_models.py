...
def test_mop_models():
    ...
    try:
        ...
        from mop.models import FuseExcInh, Kernels3, ViewsLinear
        ...
        mop_model = ViT_MoP(
            dim=256, depth=6, heads=4, n_classes=10, n_views=5, n_kernels=3
        )
        ...
        # Sanity checks
        assert baseline_out.shape == (
            batch_size,
            10,
        ), f"Expected {(batch_size, 10)}, got {baseline_out.shape}"
        assert mop_out.shape == (
            batch_size,
            10,
        ), f"Expected {(batch_size, 10)}, got {mop_out.shape}"
        ...
        print(
            f"   • Parameter counts: Baseline {baseline_params/1e6:.1f}M, MoP {mop_params/1e6:.1f}M"
        )
        ...
        print("\n💡 Make sure you:")
        print(
            "   1. Created all the model files (components.py, vit_mop.py, vit_baseline.py)"
        )
