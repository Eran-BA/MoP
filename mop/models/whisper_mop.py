...
def _sa_mask(T_q: int, T_k: int, device) -> torch.Tensor:
    ...
    return torch.tril(torch.ones(T_q, T_k, device=device, dtype=torch.bool)).view(
        1, 1, T_q, T_k
    )
...
class MultiheadCrossAttention(nn.Module):
    def __init__(
        self, dim_q: int, dim_kv: int, n_head: int, dropout: float, bias: bool
    ):
        super().__init__()
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, cfg: WhisperConfig):
        ...
        self.attn = MultiheadSelfAttention(
            D, cfg.n_head, cfg.dropout, cfg.bias, causal=False
        )
    ...
    def forward(
        self, x: torch.Tensor, mel2d: torch.Tensor
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, cfg: WhisperConfig):
        ...
        self.self_attn = MultiheadSelfAttention(
            D, cfg.n_head, cfg.dropout, cfg.bias, causal=True
        )

        self.ln2 = nn.LayerNorm(D)
        self.cross_attn = MultiheadCrossAttention(
            D, D, cfg.n_head, cfg.dropout, cfg.bias
        )
...
class WhisperMoP(nn.Module):
    ...
    def __init__(self, cfg: WhisperConfig):
        ...
        self.audio_pos = (
            nn.Embedding(cfg.n_audio_ctx, D) if cfg.use_abs_pos_emb else None
        )
        ...
        self.encoder = nn.ModuleList(
            [EncoderBlock(cfg) for _ in range(cfg.n_layer_enc)]
        )
        self.decoder = nn.ModuleList(
            [DecoderBlock(cfg) for _ in range(cfg.n_layer_dec)]
        )
    ...
    def decode(
        self, enc_out: torch.Tensor, dec_input_ids: torch.Tensor
    ) -> torch.Tensor:
        """
        enc_out: (B, T_audio, D)
