...
def test_individual_models():
    ...
    try:
        ...
        print(f"   ✅ Parameters: {sum(p.numel() for p in baseline.parameters() if p.requires_grad):,}")
        
        # Test Quartet
        print("\n📝 Testing GPT Quartet...")
        quartet = create_gpt_quartet(vocab_size, config)
        print(f"   ✅ Created: {quartet.__class__.__name__}")
        print(f"   ✅ Parameters: {sum(p.numel() for p in quartet.parameters() if p.requires_grad):,}")
        
        # Test MoP
        print("\n📝 Testing GPT MoP...")
        mop = create_gpt_mop(vocab_size, config, n_views=3, n_kernels=2)
        print(f"   ✅ Created: {mop.__class__.__name__}")
        print(f"   ✅ Parameters: {sum(p.numel() for p in mop.parameters() if p.requires_grad):,}")
