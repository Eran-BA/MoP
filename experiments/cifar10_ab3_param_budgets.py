...
def try_import_models():
    """Import Baseline/MoP and the Cross-View Mixer model."""
    # Baseline/MoP from main package
    import_attempts = [
    ...
    # Cross-View Mixer from experiments module
    try:
        from cifar10_crossview_mixer import ViTCrossView  # type: ignore

        print("✅ Imported ViTCrossView from experiments")
    except Exception as e:
        raise ImportError(f"Could not import ViTCrossView: {e}")
...
def main():
    ...
    # CrossView mixer knobs (subset; defaults keep extra features off)
    ...
    ap.add_argument("--out", type=str, default="results/cifar10_ab3_param_budgets")
    args = ap.parse_args()
    ...
    for target in args.targets:
        ...
        for s in args.seeds:
            ...
            xv = ViTCrossView(n_classes=10, **xview_cfg, **xview_extra).to(device)
            ...
            while steps < args.steps:
                ...
                if steps % max(args.eval_every, 1) == 0 or steps == 1:
                    ...
                    print(
                        f"step {steps:4d} | Lb={loss_b.item():.3f} Lm={loss_m.item():.3f} Lx={loss_x.item():.3f} | "
                        f"Ab={a_b:.3f} Am={a_m:.3f} Ax={a_x:.3f}"
                    )
        ...
        # save CSV per target
        csv_path = os.path.join(args.out, f"cifar10_ab3_target_{int(target)}.csv")
