...
def multihop_mix(
    ...
    """
    Mix two pre-softmax score tensors with logical gates and a multi-hop composition.

    Args:
        S1, S2: [B, H, T, T] pre-softmax scores (same mask/temperature)
        hops:  int >= 2, number of attention hops in the composition path
    ...
    """
    assert hops >= 2, "hops must be >= 2 for multi-hop"
    if gates is None:
    ...
    # Multi-hop (i→k1→k2→...→j) via chained row-stochastic products; add as log-prob
    if mask is not None:
        m = mask == 0
    ...
    for _ in range(hops - 1):
        C = torch.matmul(C, A2)
    eps = 1e-6
    Smix = Smix + gates["chain"] * torch.log(C + eps)
...
class MultiHopMSA(nn.Module):
    """Multi-head self-attention with dual-path logits and gated multi-hop composition.

    Value transport follows the multi-hop chain so V is aware of i→k1→…→j.
    """
    ...
    def __init__(
        ...
        # Learnable gate for multi-hop value transport mixing
    ...
    def forward(
        ...
        # Compose logits with gates (multi-hop)
        A = multihop_mix(
            S1,
            S2,
            hops=self.hops,
            mask=attn_mask,
            beta_not=self.beta_not,
            gates=self.gates,
        )
        A = self.attn_drop(A)
        ...
        # Multi-hop value transport: Y_chain = A1 @ (A2 @ (... (A2 @ V2))) with (hops-1) A2's
        ...
        for _ in range(self.hops - 1):
            transport = torch.matmul(A2, transport)
...
class BlockMultiHop(nn.Module):
    def __init__(
        ...
        self.attn = MultiHopMSA(
            dim, heads, attn_drop, drop, beta_not=beta_not, gates=gates, hops=hops
        )
...
def main():
    ap = argparse.ArgumentParser(
        description="CIFAR-10 experiment with dual-path gated attention (multi-hop composition)"
    )
    ...
    ap.add_argument("--out", type=str, default="results/cifar10_multihop_gates")
    args = ap.parse_args()
    ...
    print(f"Gates: {gates} | beta_not={args.beta_not} | hops={args.hops}")
    ...
    # Save results
    csv_path = os.path.join(args.out, "cifar10_multihop_gates.csv")
