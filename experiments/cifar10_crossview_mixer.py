...
class CrossViewMixerMSA(nn.Module):
    ...
    - Computes S1 = Q1 K1^T, S2 = Q2 K2^T
    - Cross-view scores S12 = Q1 K2^T, S21 = Q2 K1^T
    - General mixer: [Q1,Q2] M [K1;K2]^T => linear mix of {S1,S12,S21,S2}
    - Optional transpose channels S1^T and S2^T with weights t1, t2
    - Optional per-key prior: A_sharp(i,j) ∝ A1(i,j) * A2(k*, j) then row-normalize
    """
    ...
    def __init__(
        ...
        anchor_mode: str = "argmax_row_sum",  # or "fixed" or "none"
        fixed_k_star: int = 0,
    ):
        super().__init__()
        assert dim % heads == 0
    ...
    def _compute_logits(
        ...
        q1, k1, v1 = qkv1[0], qkv1[1], qkv1[2]  # [B,H,N,dk]
    ...
    def forward(
        ...
        if self.enable_per_key_prior and self.prior_weight > 0.0:
            # Build A_sharp from A1 sharpened by A2 anchor row k*
            S1m = self._apply_mask(S1, attn_mask)
            S2m = self._apply_mask(S2, attn_mask)
            A1 = F.softmax(S1m, dim=-1)  # [B,H,N,N]
            ...
            if self.anchor_mode == "fixed":
                ...
                # pick k* maximizing row-sum of A2 per (B,H)
                row_sum = A2.sum(dim=-1)  # [B,H,N]
                k_star = row_sum.argmax(dim=-1)  # [B,H]
            else:
                k_star = torch.zeros((B, self.h), device=x.device, dtype=torch.long)

            # Gather anchor row from A2: shape [B,H,1,N]
...
def main():
    ap = argparse.ArgumentParser(
        description="CIFAR-10 cross-view mixer with per-key prior sharpening and transpose cues"
    )
    ...
    ap.add_argument("--out", type=str, default="results/cifar10_crossview_mixer")
    args = ap.parse_args()
    ...
    csv_path = os.path.join(args.out, "cifar10_crossview_mixer.csv")
