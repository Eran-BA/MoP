...
def try_import_twohop():
    try:
        from cifar100_twohop_gates import ViTGated  # type: ignore

        print("✅ Imported ViTGated (two-hop, CIFAR-100) from experiments")
        return ViTGated
    except Exception as e:
        raise ImportError(f"Could not import ViTGated from cifar100_twohop_gates: {e}")
...
def main():
    ap = argparse.ArgumentParser(
        description="Param-match TwoHop ViT on CIFAR-100 at specified budgets"
    )
    ...
    ap.add_argument("--out", type=str, default="results/cifar100_twohop_param_budgets")
    args = ap.parse_args()
    ...
    for target in args.targets:
        ...
        for s in args.seeds:
            ...
            set_seed(s)
            model = ViTGated(
                n_classes=100, **cfg, gates=gates, beta_not=args.beta_not
            ).to(device)
        ...
        csv_path = os.path.join(args.out, f"cifar100_twohop_target_{int(target)}.csv")
